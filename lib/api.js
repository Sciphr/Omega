  ...
  async fetchUserTeams() {
    ...
    const allTeams = (data.teams || []).map(team => {
      ...
      if (team.captain_id === currentUserId) {
        ...
        // Check if user is in member_details as a member
        const memberRecord = team.member_details?.find(member =>
          member.user_id === currentUserId && member.role !== 'captain'
        )
        if (memberRecord) {
          userRole = memberRecord.role === 'leader' ? 'leader' : 'member'
        }
      }
      ...
      return {
        ...
        game: team.game,
        role: userRole,
        members: team.member_details?.length || 1,
