...
export class TournamentService {
  static async createTournament(tournamentData, creatorId = null) {
    try {
      ...
      const tournament = {
        ...
        round_robin_type: tournamentData.roundRobinType || null,
        group_count: tournamentData.groupCount || null,
        group_creation_method: tournamentData.groupCreationMethod || null,
        ...
        thumbnail_image_url: tournamentData.thumbnailImageUrl || null,
        banner_image_url: tournamentData.bannerImageUrl || null,
      ...
      // Track tournament creation achievement
      if (creatorId) {
        await AchievementService.trackTournamentCreation(creatorId, data)
      }
  ...
  static async joinTournament(tournamentId, participantData, password = null) {
    try {
      ...
      // Track tournament participation achievement
      if (finalUserId) {
        await AchievementService.trackTournamentParticipation(finalUserId, tournament)
      }
    ...
    try {
      ...
      if (tournament.format === 'single_elimination') {
        ...
        const roundRobinOptions = {
          type: tournament.round_robin_type || 'single',
          groupCount: tournament.group_count,
          groupCreationMethod: tournament.group_creation_method
        }

        bracket = await BracketGenerator.generateRoundRobinBracket(seededParticipants, roundRobinOptions)

        if (!bracket) {
          throw new Error('Failed to generate round robin bracket')
        }
        ...
        if (bracket.matches) {
          bracket.matches.forEach(match => {
            matches.push({
              ...
              tournament_id: tournamentId,
              round: match.round || 1,
              match_number: match.matchNumber || match.number,
              bracket_type: 'group',
              group_id: match.group_id || match.groupId,
              participant1_id: match.participant1?.id || null,
              participant2_id: match.participant2?.id || null,
              winner_id: match.winner_id || null,
              status: match.status || MATCH_STATUS.PENDING,
              match_format: tournament.settings?.matchFormat || 'bo1'
            })
          })
        }
