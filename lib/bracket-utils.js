import { TOURNAMENT_FORMAT, MATCH_STATUS, PARTICIPANT_STATUS } from './types'

export class BracketGenerator {
  // Generate bracket structure from database matches
  static generateBracketFromMatches(matches, participants) {
  ...
  static generateLoserBracket(participantCount) {
    const rounds = []
    let eliminatedFromWinner = Math.floor(participantCount / 2)
    let roundNumber = 1
    
    // Loser bracket has a complex structure
    // This is a simplified version - full implementation would be more complex
    while (eliminatedFromWinner > 1) {
      const matches = []
      const matchCount = Math.ceil(eliminatedFromWinner / 2)
      ...
      eliminatedFromWinner = Math.ceil(eliminatedFromWinner / 2)
      roundNumber++
    }
...
export class TournamentManager {
  ...
  static seedParticipants(participants, seedingType) {
    switch (seedingType) {
      case 'random':
      ...
      case 'ranked':
        return participants.sort((a, b) => (b.ranking || 0) - (a.ranking || 0))
      default:
        return participants
    }
  }
